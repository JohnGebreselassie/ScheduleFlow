...
class Simulator():
    ...
    def create_scenario(self, scenario_name, scheduler, job_list=[]):
        ...
        self.stats = _intScheduleFlow.StatsEngine(
                self.__system.get_available_nodes())#small change edit
        if self.__generate_gif:
            self.__viz_handler = _intScheduleFlow.VizualizationEngine(
                    self.__system.get_available_nodes())#changing numbers in the visualization engine changes max nodes here
    ...
    def __sainity_check_schedule(self, workload):
        ...
        for i in range(len(event_list) - 1):
            ...
            if procs > self.__system.get_available_nodes():
                check_fail += 1
        return check_fail
...
class System(object):
    ...
    def __init__(self, total_nodes, availability_fn=None):
        ...
        self.__availability_fn = availability_fn
    ...
    def get_available_nodes(self):
        if self.__availability_fn:
            return self.__availability_fn(self.__current_time)
        return self.__total_nodes
    ...
    def start_job(self, nodes, jobid, timestamp=None):
        ''' Method for aquiring resources in the system '''

        self.__free_nodes -= nodes
        if timestamp is not None:
            print(f"[Time {timestamp}] Starting job {jobid} using {nodes} nodes")
        else:
            print(f"Starting job {jobid} using {nodes} nodes")
...
class Scheduler(object):
    ...
    def submit_job(self, job):
        ...
        assert (job.nodes <= self.system.get_available_nodes()),\
    ...
    def allocate_job(self, job):
        ...
        self.system.start_job(job.nodes, job.job_id, self.system.get_current_time())
    ...
    def create_gaps_list(self, reserved_jobs, min_ts):
        ...
        for event in order_reservations:
            free_nodes = self.system.get_available_nodes() - nodes
...
class OnlineScheduler(Scheduler):
    ...
    def trigger_schedule(self):
        ...
        if len(self.wait_queue) == 0:
            return []
        free_nodes = self.system.get_available_nodes()
        while free_nodes > 0:
